...
import { AuthSetupModal } from '@/components/modals/AuthSetupModal';
...
export const MainMenu: React.FC<MainMenuProps> = ({
  ...
  return (
    <div className="w-full h-full min-h-[100dvh] flex flex-col justify-between items-center px-4 py-4 sm:py-6 max-w-md mx-auto select-none">
          ...
          {user ? (
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-zinc-900/90 hover:bg-zinc-800 text-zinc-300 border border-zinc-800 text-xs font-semibold shadow-sm transition-all tap-bounce"
              >
                <span className="w-4 h-4 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-[10px] font-bold text-sky-400">
                  {profile.name[0]}
                </span>
                <span className="max-w-[70px] truncate">{profile.name}</span>
              </button>
              ...
              >
                ...
                <span>{signingIn ? 'Connecting...' : 'Google'}</span>
              </button>
            </div>
          )}
        </div>
      ...
      {/* Google OAuth & Custom Player Name Setup Modal */}
      <AuthSetupModal
